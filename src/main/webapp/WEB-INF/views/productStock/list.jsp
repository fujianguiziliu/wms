...
<!--                         <input type="button" value="查询" class="ui_input_btn01 btn_page"  data-page="1"/>
 -->
                        <input type="button" value="新增" class="ui_input_btn01 btn_input"
                               data-url="<s:url namespace="/" action="productStock_input"/>"/>
                      <%--  <input type="button" value="批量删除" class="ui_input_btn01 btn_batchDelete"
                               data-url="<s:url namespace="/" action="productStock_batchDelete"/>"/> --%>
                    </div>
                </div>
            </div>
        </div>
                       ...
                       <%-- 定义一个变量,接收数据模型里面的表达式--%>
                            <%--h?keys: h是一个map结构的数据,代表获取到h里面的key的集合--%>
                                <%--遍历keys集合,把每次遍历的数据赋值给变量 key--%>
                                        <%--库存总金额:从map里面去去key对应的数据信息:中文名称--%>
                                    <th>库存总金额</th>
                                        <%--货品:从map里面去去key对应的数据信息:中文名称--%>
                                    <th>货品</th>
                                        <%--库存数量:从map里面去去key对应的数据信息:中文名称--%>
                                    <th>库存数量</th>
                                        <%--库存价格:从map里面去去key对应的数据信息:中文名称--%>
                                    <th>库存价格</th>
                    ...
                    <s:iterator value="#result.listData">
                        <tr>
                            ...
                            <td><s:property value="id"/></td>
                                ...
                                <td><s:property value="depot"/> </td>
                            <td>
                                <s:a namespace="" action="productStock_input">
                                    <s:param name="productStock.id" value="id"></s:param>
                                    编辑</s:a>
                                <s:url namespace="" action="productStock_delete" var="deleteUrl">
                                    <s:param name="productStock.id" value="id"></s:param>
                                </s:url>
